...
const DEFAULT_CAST_MISC = { features: "" };
const mergeCastDefaults = (c) => ({ ...DEFAULT_CAST_PAY, ...DEFAULT_CAST_COUNTERS, ...DEFAULT_CAST_MISC, ...c });
...
export default function App() {
  ...
  return (
    <div style={{ background: "#000", minHeight: "100vh", color: "#fff", fontFamily: "system-ui, sans-serif" }} className="pb-24">
      ...
      {view === "floor" && <Floor {...{ visibleTables, dispTable, tables, ts, castById, setSel, merges, mergeGroups, toggleMerge, casts, busy }} />}
      ...
      <div style={{ background: "#0a0a0c", borderTop: "1px solid #1c1c22" }} className="fixed bottom-0 inset-x-0 z-30 flex">
        {[["floor", LayoutGrid, "フロア"], ["cast", Sparkles, "キャスト"], ["sales", null, "売上"], ["admin", Sun, "設定"]].map(([k, Icon, label]) => (
...
function Floor({ visibleTables, dispTable, tables, ts, castById, setSel, merges, mergeGroups, toggleMerge, casts, busy }) {
  const groupEntries = Object.entries(mergeGroups || {});
  const totalCustomers = Object.values(ts).filter(t => t?.active).reduce((s, t) => s + (t.customers?.length || 0), 0);
  const workingCasts = busy ? busy.size : 0;
  const standbyCasts = (casts || []).filter(c => c.clockedInAt && !busy?.has(c.id)).length;
  return (
    <div className="p-3">
      <div className="flex items-center justify-center gap-4 mb-3 text-[13px]">
        <span className="flex items-center gap-1.5 text-zinc-400"><Users size={13} />客 <span className="text-white font-bold">{totalCustomers}</span></span>
        <span className="text-zinc-700">|</span>
        <span className="flex items-center gap-1.5 text-zinc-400">接客 <span style={{ color: GOLD }} className="font-bold">{workingCasts}</span></span>
        <span className="text-zinc-700">|</span>
        <span style={{ background: "rgba(63,182,176,.15)", border: `1px solid ${TEAL}`, color: TEAL }} className="rounded-full px-3 py-0.5 font-bold text-[12px]">待機 {standbyCasts}</span>
      </div>
  ...
  return (
    <div style={{ background: "#141418", border: "1px solid #22222a" }} className="rounded-xl p-3">
      ...
      <div className="flex items-start gap-2 mb-2">
        <span className="text-[10px] text-zinc-500 w-12 mt-2">特徴</span>
        <input value={c.features || ""} onChange={e => upd(c.id, x => ({ ...x, features: e.target.value }))} placeholder="例: 抜け遅い→5分前に抜く / お酒弱い" style={{ background: "#0d0d10", border: "1px solid #22222a", fontSize: "14px" }} className="flex-1 rounded px-2 py-1.5 outline-none min-w-0" />
      </div>
